...
#include "shr_assert.h"
  ...
  use clm_varctl      , only : use_cndv, iulog, use_luna, use_crop, use_fates_hydrohard,use_fates_frosthard !marius
...
contains
    ...
    associate(snl => col%snl) ! Output: [integer (:)    ]  number of snow layers
      ...
      do p = bounds%begp, bounds%endp
         ...
         if (use_vancouver) then
            ...
            !this%t_ref2m_24_patch(p) = 273.15_r8 marius
  ...
  subroutine InitAccBuffer (this, bounds)
    ...
    if ( use_crop )then
       ...
       call init_accum_field (name='TDM5', units='K', &
            desc='5-day running mean of min 2-m temperature', accum_type='runmean', accum_period=-365*5, &
            subgrid_type='pft', numlev=1, init_value=SHR_CONST_TKFRZ)
    end if
  ...
  subroutine UpdateAccVars (this, bounds)
    ...
    use clm_time_manager , only : is_end_curr_year !marius
